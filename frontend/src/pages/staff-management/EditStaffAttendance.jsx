...
import { getAttendanceById, updateAttendance } from '../../services/staffAttendanceService';
...
const EditStaffAttendance = () => {
  ...
  // Fetch attendance, staff, and routes on component mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const attendanceResponse = await getAttendanceById(id);
        if (attendanceResponse.success) {
          setFormData({
            staff_id: attendanceResponse.data.staff_id || '',
            attendance_date: attendanceResponse.data.attendance_date.split('T')[0] || '',
            role: attendanceResponse.data.role || '',
            route_id: attendanceResponse.data.route_id || '',
            attendance_status: attendanceResponse.data.attendance_status || 'present',
            check_in_time: attendanceResponse.data.check_in_time || '',
            check_out_time: attendanceResponse.data.check_out_time || '',
            leave_type: attendanceResponse.data.leave_type || '',
          });
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await updateAttendance(id, dataToSend);
      if (response.success) {
  ...
  return (
    <>
     <HeadTags title="Edit Staff Attendance" />
     <TopProgressBar loading={loading} />
      <div className="page-header mb-30 px-2">
        <div className="page-title mb-3">
          <h3 className="fs-30">Edit Staff Attendance</h3>
        </div>
      ...
      <div className="row justify-content-center mb-5">
        <div className="col-lg-10 col-xl-8">
          <div className="card p-25">
            <h3 className="fw-600 fs-18 mb-4">Edit Staff Attendance Information {formData.check_out_time}</h3>
